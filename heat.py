...
def heatmap_year(ax, pivot, title):
...
def main():
    ...
    for yr in [2021, 2025]:
        row = wide.loc[[yr]].copy() if yr in wide.index else None
        if row is None or row.empty:
            raise RuntimeError(f"Year {yr} not found in data for heatmap.")
        pivot = row.copy()
        ...
        plt.xlabel("Party")
        plt.ylabel("Year")
        plt.title(f"{yr} vote share heatmap (single-year)")
        plt.colorbar(im, label="Vote share (%)")
        plt.tight_layout()
        plt.savefig(os.path.join(OUT_DIR, f"fig3_heatmap_{yr}.png"), dpi=160)
        plt.close()
